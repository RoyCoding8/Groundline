...
def resolve_openai_compat_policy(
    *,
    ...
    2. ``DISTORTION_MODEL`` environment variable.
    3. :class:`ValueError` if neither is available.

    Other settings are read from the environment:

    - ``DISTORTION_API_BASE`` / ``DISTORTION_API_KEY``
    - ``DISTORTION_TIMEOUT_SECONDS`` (default 120)
    - ``DISTORTION_MAX_ATTEMPTS`` (default 2)
    """
    resolved_model = (model or "").strip() or os.environ.get("DISTORTION_MODEL", "").strip()
    if not resolved_model:
        raise ValueError("model is required: supply --model / request body or set DISTORTION_MODEL")

    api_base = os.environ.get("DISTORTION_API_BASE", "").strip() or None
    api_key = os.environ.get("DISTORTION_API_KEY", "").strip() or None

    timeout_str = os.environ.get("DISTORTION_TIMEOUT_SECONDS", "").strip()
    timeout = float(timeout_str) if timeout_str else 120.0

    attempts_str = os.environ.get("DISTORTION_MAX_ATTEMPTS", "").strip()
    ...
    *,
    ...
    if name in {"record", "locked"}:
        ...
        if artifacts is not None:
            resolved = (model or "").strip() or os.environ.get("DISTORTION_MODEL", "").strip()
            api_base = os.environ.get("DISTORTION_API_BASE", "").strip() or None
