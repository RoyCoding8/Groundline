...
def create_app(artifacts: Path = Path("artifacts")) -> FastAPI:
    ...
    app = FastAPI(title="The Groundline API", version=__version__)
    app.add_middleware(
        CORSMiddleware,
    ...
    def run_decisions(run_id: str) -> dict[str, Any]:
        directory = _run_directory(repository, run_id)
        return {"run_id": run_id, "nodes": _decision_nodes(directory)}
    ...
    async def launch_experiment(launch: LaunchExperiment) -> JobStatus:
        ...
        if launch.policy != "fixture":
            ...
            if not effective_model:
                raise HTTPException(
                    status_code=503,
                    detail="model is required: supply request body or set GROUNDLINE_MODEL",
                )
        try:
...
def _decision_nodes(directory: Path) -> list[dict[str, Any]]:
    """Project the per-agent decision ledger into flat, serializable nodes.

    Reuses verify_run_artifacts(), which already loads and validates
    decisions.jsonl. Each entry's raw decision dict is re-validated through
    PolicyDecision so the agent's reported reasoning (report.explanation) is
    surfaced. A single malformed entry is skipped rather than aborting the
    whole listing — a corrupt artifact already surfaces as a typed 500 from
    verify_run_artifacts via _run_directory.
    """
    artifacts = verify_run_artifacts(directory)
    nodes: list[dict[str, Any]] = []
    for entry in artifacts.decisions:
        try:
            decision = PolicyDecision.model_validate(entry.decision)
...
# Module-level ASGI app so `uvicorn groundline.api.app:app` (used by the
