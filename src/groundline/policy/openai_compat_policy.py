...
from groundline.events.store import canonical_json
from groundline.policy.models import (
    AgentContext,
    AgentMemory,
    AgentOutput,
    PolicyDecision,
...
async def _retry_backoff_delay(attempt: int) -> float:
    ...
    Reads the base delay from ``GROUNDLINE_RETRY_BACKOFF_SECONDS`` when set so
    ...
    if attempt <= 0:
        return 0.0
    env_base = os.environ.get("GROUNDLINE_RETRY_BACKOFF_SECONDS", "").strip()
...
class OpenAICompatPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        # mapping OR GROUNDLINE_TEMPERATURE. If generation_parameters sets
        # "temperature", it wins (it is merged after this in the body).
        env_temp = os.environ.get("GROUNDLINE_TEMPERATURE", "").strip()
        explicit_temp = None
    ...
    async def _record_decision(
        ...
        # .update() below) or GROUNDLINE_TEMPERATURE override it.
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                try:
                    ...
                    if os.environ.get("GROUNDLINE_DEBUG_SCHEMA", "") == "1":
